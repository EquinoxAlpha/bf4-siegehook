...
#define OFFSET_SYNCEDSETTINGS		  0x1423717C0
...
public:
	...
	mat4 m_VehicleMatrix; //0x0030
	mat4 m_SoldierMatrix; //0x0070
	float m_VehicleYawFixed; //0x00B0
	vec2 m_Angles; //0x00B4
	float m_ZeroFloat; //0x00BC Always == zero
	bool m_SomeBool; //0x00C0

	vec3 GetVehicleCameraOrigin();
	vec3 GetVehicleCrosshair();
}; //Size: 0x011C
...
public:
	...
	mat4 m_View; //0x0260 
	mat4 m_ViewTranspose; //0x02A0 
	mat4 m_ViewInverse; //0x02E0 
	mat4 m_Projection; //0x0320 
	mat4 m_ViewAtOrigin; //0x0360 
	mat4 m_ProjectionTranspose; //0x03A0 
	mat4 m_ProjectionInverse; //0x03E0 
	mat4 m_ViewProjection; //0x0420 
	mat4 m_ViewProjectionTranspose; //0x0460 
	mat4 m_ViewProjectionInverse; //0x04A0 
...
public:
	...
	char m_Settings[0x20];//SystemSettings m_Settings; //0x0000
...
public:
	...
	int32_t m_Faction; //0x0020 Enum FactionId (included in TypeInfo)
	char pad_0024[4]; //0x0024
	VeniceSoldierCustomizationAsset** m_ppSoldierCustomization; //0x0028
	VeniceVehicleCustomizationAsset** m_ppVehicleCustomization; //0x0030
}; //Size: 0x0038
...
public:
	...
	TeamData* m_pTeam; //0x0018
	int32_t m_Id; //0x0020 Enum TeamId (included in TypeInfo)
	char pad_0024[4]; //0x0024
}; //Size: 0x0028
...
public:
	...
	TeamData** m_ppTeamEntityData; //0x0040 
	char* m_LevelName;
...
public:
	...
	virtual bool PhysicsRayQuery(const char* identifier, vec3* from, vec3* to, RayCastHit* hit, int flag, eastl::fixed_vector<void const*, 8, 0>* PhysicsEntityList);
};//Size=0x0008
...
public:
	...
	inline ClientVehicleEntity* GetControlledVehicle()
	{
		if (this->IsInVehicle())
			return reinterpret_cast<ClientVehicleEntity*>(m_pControlledControllable);
...
{
public:
	class CustomizationTable* m_pCustomization; //0x0018
}; //Size: 0x0020

class VeniceVehicleCustomizationAsset : public VehicleCustomizationAsset
{
public:
	int32_t m_Category; //0x0020 Enum VehicleCategory (included in TypeInfo)
	char pad_0024[4]; //0x0024
	char* m_NameSid; //0x0028
	int32_t m_SpacerIndex; //0x0030
...
public:
	char pad_0000[16]; //0x0000
	char* m_UICategorySid; //0x0010
	uint32_t m_DefaultSelectionIndex; //0x0018
	char pad_001C[4]; //0x001C
	UnlockAssetBase** m_ppSelectableUnlocks; //0x0020
}; //Size: 0x0028
...
public:
	char _0x0000[16];
	eastl::vector<CustomizationUnlockParts*> m_pUnlockParts; //0x0010
	char _0x0018[40];
...
public:
	...
	virtual void GetTransform(mat4* mTransform);
	...
	inline vec3 GetPosition() {
		...
		this->GetAABB(&aabb);
		return aabb.Transform.position();
	}
	...
	inline vec3 GetCenter() {
		...
		vec3 position = aabb.Transform.position();
...
public:
	char _0x0000[0x38-4];
	float m_breathControlTimer;					// 0x38
	float m_breathControlMultiplier;			// 0x3c
	float m_breathControlPenaltyTimer;			// 0x40
	float m_breathControlPenaltyMultiplier;		// 0x44
	float m_breathControlActive;				// 0x48
};
...
public:
	...
	BreathControlHandler* m_pBreathControlHandler; //0x0588
	char _0x0588[32];
...
public:
	...
	mat4 m_ShootSpace; //0x0040 
	mat4 m_ShootSpaceIdentity; //0x0080 
	char _0x00C0[464];
...
{
public:
	vec4 m_PositionOffset; //0x0000 
	vec4 m_Direction; //0x0010 
	vec4 m_Speed; //0x0020 
	char _0x0030[32];
	...
	bool GetBone(const UpdatePoseResultData::BONES BoneId, vec3& BoneOut)
	{
		__try
		{
			...
			if (PoseResult.m_ValidTransforms)
			{
				...
				BoneOut = vec3(Bone.x, Bone.y, Bone.z);
				return true;
			}
			else
			{
...
{
public:
	char _0x0000[1088-0x10];
	float m_RecoilSight; // 0x430
	float m_SpreadSight; // 0x434
	float m_RecoilHip;   // 0x438
	float m_SpreadHip;   // 0x43C
...
public:
	...
	virtual void GetTransform(mat4* mTransform); //23
	...
	vec3 N000000A4; //0x0260
	vec3 N000000A5; //0x026C
	vec3 N000000A6; //0x0278
	vec3 N000000A7; //0x0284
	vec3 m_Position; //0x0290
};
...
public:
	...
	void drawText(const vec2& t_screenpos, Color32 t_color, const char* t_text, float t_scale = 1.0f)
	{
	...
	void drawLine2d(const vec2& point_1, const vec2& point_2, Color32 color)
	{
	...
	void drawBox2d(vec2 from, vec2 to, float width, Color32 color) {
